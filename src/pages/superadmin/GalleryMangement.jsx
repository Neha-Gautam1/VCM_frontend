...
import { mockGalleryImages as initialImages, galleryCategories } from "../../data/mockGallery";
...
const GalleryManagement = () => {
  ...
  const handleUploadSubmit = (e) => {
    e.preventDefault();
    const randomImg = sampleUploadImages[Math.floor(Math.random() * sampleUploadImages.length)];
    setImages([{ id: Date.now(), url: randomImg, title: form.title, category: form.category, uploadedOn: new Date().toISOString().split("T")[0] }, ...images]);
    setForm({ title: "", category: "Architecture" });
    setUploadOpen(false);
  };

  const handleDeleteConfirm = () => {
    setImages(images.filter((img) => img.id !== activeImage.id));
    setDeleteOpen(false);
  ...
  return (
    <DashboardLayout menuItems={superAdminMenuItems} pageTitle="Gallery Management" profilePath="/superadmin/profile" settingsPath="/superadmin/settings">
        ...
        {galleryCategories.map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-colors ${
              category === cat ? "bg-gradient-to-r from-saffron-600 to-maroon-600 text-white shadow-soft" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"
            }`}
          >
            {cat}
          </button>
      ...
      {/* Upload Modal */}
      <Modal
        isOpen={uploadOpen} onClose={() => setUploadOpen(false)} title="Upload New Image" size="sm"
        footer={<>
      ...
      >
        <form onSubmit={handleUploadSubmit} className="space-y-4">
          <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:border-saffron-300 transition-colors cursor-pointer">
            <FaUpload className="text-2xl text-slate-300 mx-auto mb-2" />
            <p className="text-sm text-slate-500">Click to browse or drag & drop</p>
            <p className="text-xs text-slate-400 mt-1">PNG, JPG up to 5MB (prototype — no real upload)</p>
          </div>
          <div>
